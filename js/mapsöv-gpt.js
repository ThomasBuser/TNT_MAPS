...
window.initMap = function() {
    ...
    window.setTravelMode = function(mode) {
        ...
        if (mode === "DRIVING") {
            ...
            if (window.currentLocation && window.destination) {
                calculateAndDisplayRoute(window.currentLocation, window.destination, false);
            }
        } else {
            stopNavigation();
            if (window.currentLocation && window.destination) {
        ...
        window.directionsService.route(
            ...
            function(response, status) {
                if (status === "OK") {
                    ...
                    // Show transit popup if travelMode is TRANSIT
                    if (window.travelMode === google.maps.TravelMode.TRANSIT) {
                        showTransitPopup(response);
                    }
                } else {
    ...
    function showTransitPopup(response) {
        const steps = response.routes[0].legs[0].steps;
        const popup = document.createElement('div');
        popup.id = 'transitInstructionPopup';
        popup.style.cssText = `
            position: fixed;
            top: 50%;
            left: 50%;
        ...
        let content = '<h3 style="margin-top:0;color:#1976D2;">ÖV Navigation</h3><ol style="padding-left:20px;">';
        ...
        for (const step of steps) {
            if (step.travel_mode === "TRANSIT") {
                ...
                content += `
                    <li>
                        Mit <strong>${transit.line.vehicle.name} ${transit.line.short_name}</strong><br>
                        Von <strong>${transit.departure_stop.name}</strong> (${new Date(transit.departure_time.value).toLocaleTimeString('de-CH', {hour: '2-digit', minute: '2-digit'})})<br>
                        Nach <strong>${transit.arrival_stop.name}</strong> (${new Date(transit.arrival_time.value).toLocaleTimeString('de-CH', {hour: '2-digit', minute: '2-digit'})})
                    </li>
                `;
            } else {
                const cleanInstruction = step.instructions.replace(/<[^>]*>/g, '');
                content += `<li>${cleanInstruction}</li>`;
            }
        }

        content += '</ol><div style="text-align:center;margin-top:10px;"><button onclick="document.getElementById(\'transitInstructionPopup\').remove()" style="background:#f44336;color:white;border:none;padding:8px 16px;border-radius:4px;cursor:pointer;">Schließen</button></div>';
    ...
    // function showTransitOptions(from, to) {
    //     const transitPanel = document.createElement('div');
    //     transitPanel.id = 'transitPanel';
    //     transitPanel.style.cssText = `
    //         position: absolute;
    //         top: 10px;
    //         left: 10px;
    //         right: 10px;
    ...
    // 
    //     const now = new Date();
    //     const timeString = now.toLocaleTimeString('de-CH', { hour: '2-digit', minute: '2-digit' });
    // 
    //     const departures = [
    //         { line: 'Bus 10', departure: addMinutes(now, 3), fromStation: 'Bahnhofplatz', toStation: 'Zytglogge' },
    //         { line: 'Tram 9', departure: addMinutes(now, 8), fromStation: 'Hirschengraben', toStation: 'Universität' },
    //         { line: 'Bus 20', departure: addMinutes(now, 14), fromStation: 'Kocherpark', toStation: 'Wankdorf Center' }
    //     ];
    // 
    //     const departuresHtml = departures.map(d => `
    //         <li>
    //             <strong>${d.line}</strong> – Abfahrt um ${d.departure.toLocaleTimeString('de-CH', { hour: '2-digit', minute: '2-digit' })}<br>
    //             <small>Einsteigen: ${d.fromStation} – Aussteigen: ${d.toStation}</small>
    //         </li>
    //     `).join('');
    // 
    //     transitPanel.innerHTML = `
    //         <h3 style="margin-top: 0; color: #1976D2;">ÖV-Verbindungen</h3>
    //         <ul style="padding-left: 20px;">${departuresHtml}</ul>
    //         <button onclick="document.getElementById('transitPanel').remove()" style="background:#f44336;color:white;border:none;padding:6px 12px;border-radius:4px;cursor:pointer;">Schließen</button>
    //     `;
    // 
    //     // Remove previous panel if it exists
    //     const existingPanel = document.getElementById('transitPanel');
    //     if (existingPanel) existingPanel.remove();
    // 
    //     document.getElementById('map').appendChild(transitPanel);
    // 
